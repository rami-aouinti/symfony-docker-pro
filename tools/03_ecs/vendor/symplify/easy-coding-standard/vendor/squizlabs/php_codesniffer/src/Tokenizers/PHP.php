...
class PHP extends \PHP_CodeSniffer\Tokenizers\Tokenizer
{
     ...
     * @var array
     */
    public $endScopeTokens = [T_CLOSE_CURLY_BRACKET => T_CLOSE_CURLY_BRACKET, \T_ENDIF => \T_ENDIF, \T_ENDFOR => \T_ENDFOR, \T_ENDFOREACH => \T_ENDFOREACH, \T_ENDWHILE => \T_ENDWHILE, \T_ENDSWITCH => \T_ENDSWITCH, \T_ENDDECLARE => \T_ENDDECLARE, \T_BREAK => \T_BREAK, \T_END_HEREDOC => \T_END_HEREDOC, T_END_NOWDOC => T_END_NOWDOC];
    /**
        ...
        for ($stackPtr = 0; $stackPtr < $numTokens; $stackPtr++) {
            ...
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                if ($tokenIsArray === \true) {
                    ...
                    $content = \PHP_CodeSniffer\Util\Common::prepareForOutput($token[1]);
                } else {
                    $newToken = self::resolveSimpleToken($token[0]);
                    $type = $newToken['type'];
                    $content = \PHP_CodeSniffer\Util\Common::prepareForOutput($token[0]);
                }
                echo "\tProcess token ";
                if ($tokenIsArray === \true) {
                    echo "[{$stackPtr}]";
            ...
            //end if
            if ($newStackPtr > 0 && isset(\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$finalTokens[$newStackPtr - 1]['code']]) === \false) {
            ...
            */
            if ($tokenIsArray === \true && \substr($token[1], -1) === "\r") {
                if (isset($tokens[$stackPtr + 1]) === \true && \is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][1][0] === "\n") {
                    $token[1] .= "\n";
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                        if ($isWin === \true) {
                            echo '\\n';
            ...
            */
            if ($tokenIsArray === \true && ($token[0] === \T_DOC_COMMENT || $token[0] === \T_COMMENT && \strpos($token[1], '/**') === 0)) {
            ...
            */
            if (\PHP_VERSION_ID >= 80000 && $tokenIsArray === \true && ($token[0] === \T_COMMENT && (\strpos($token[1], '//') === 0 || \strpos($token[1], '#') === 0)) && isset($tokens[$stackPtr + 1]) === \true && \is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === \T_WHITESPACE) {
            ...
            */
            if ($tokenIsArray === \false && ($token[0] === '"' || $token[0] === 'b"')) {
                ...
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                    ...
                    if ($subTokenIsArray === \false && $subToken[0] === '"' && empty($nestedVars) === \true) {
            ...
            */
            ...
            /*
                ...
                alone though.
            */
            if ($tokenIsArray === \true && $token[0] === \T_START_HEREDOC) {
                ...
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                    ...
                    if ($subTokenIsArray === \true && $tokens[$i][0] === \T_END_HEREDOC) {
            ...
            /*
                ...
                in PHP 5, 7 and 8.
            */
            if (\PHP_VERSION_ID >= 80000 && $tokenIsArray === \true && ($token[0] === \T_NAME_QUALIFIED || $token[0] === \T_NAME_FULLY_QUALIFIED || $token[0] === \T_NAME_RELATIVE)) {
                $name = $token[1];
            ...
            */
            if (\PHP_VERSION_ID < 70000 && \PHP_VERSION_ID >= 50500 && $tokenIsArray === \true && $token[0] === \T_YIELD && isset($tokens[$stackPtr + 1]) === \true && isset($tokens[$stackPtr + 2]) === \true && $tokens[$stackPtr + 1][0] === \T_WHITESPACE && $tokens[$stackPtr + 2][0] === \T_STRING && \strtolower($tokens[$stackPtr + 2][1]) === 'from') {
                ...
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    for ($i = $stackPtr + 1; $i <= $stackPtr + 2; $i++) {
                        $type = \PHP_CodeSniffer\Util\Tokens::tokenName($tokens[$i][0]);
                        $content = \PHP_CodeSniffer\Util\Common::prepareForOutput($tokens[$i][1]);
            ...
            */
            if (\PHP_VERSION_ID < 50500 && $tokenIsArray === \true && $token[0] === \T_STRING && \strtolower($token[1]) === 'yield') {
                if (isset($tokens[$stackPtr + 1]) === \true && isset($tokens[$stackPtr + 2]) === \true && $tokens[$stackPtr + 1][0] === \T_WHITESPACE && $tokens[$stackPtr + 2][0] === \T_STRING && \strtolower($tokens[$stackPtr + 2][1]) === 'from') {
            ...
            */
            if ($tokenIsArray === \false && $token[0] === '.' && isset($tokens[$stackPtr + 1]) === \true && isset($tokens[$stackPtr + 2]) === \true && $tokens[$stackPtr + 1] === '.' && $tokens[$stackPtr + 2] === '.') {
                $newToken = [];
            ...
            /*
                ...
                these tokens in earlier versions.
            */
            if ($tokenIsArray === \false && $token[0] === '*' && isset($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1] === '*') {
                $newToken = [];
                $newToken['code'] = \T_POW;
                $newToken['type'] = 'T_POW';
                $newToken['content'] = '**';
            ...
            /*
                ...
                these tokens in earlier versions.
            */
            if ($tokenIsArray === \false && $token[0] === '*' && isset($tokens[$stackPtr + 1]) === \true && \is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][1] === '*=') {
                $newToken = [];
            ...
            */
            ...
            /*
                ...
                So look for and combine these tokens in earlier versions.
            */
            if ($tokenIsArray === \false && $token[0] === '?' && isset($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === '?') {
                $newToken = [];
            ...
            /*
                ...
                So look for and combine these tokens in earlier versions.
            */
            if ($tokenIsArray === \false && $token[0] === '?' && isset($tokens[$stackPtr + 1]) === \true && \is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === \T_OBJECT_OPERATOR) {
                $newToken = [];
            ...
            */
            if (\PHP_VERSION_ID < 70400 && ($tokenIsArray === \true && ($token[0] === \T_LNUMBER || $token[0] === \T_DNUMBER) && isset($tokens[$stackPtr + 1]) === \true && \is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === \T_STRING && $tokens[$stackPtr + 1][1][0] === '_')) {
                $newContent = $token[1];
                $newType = $token[0];
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                ...
                //end for
                if ($newType === \T_LNUMBER && (\stripos($newContent, '0x') === 0 && \hexdec(\str_replace('_', '', $newContent)) > \PHP_INT_MAX || \stripos($newContent, '0b') === 0 && \bindec(\str_replace('_', '', $newContent)) > \PHP_INT_MAX || (\stripos($newContent, '0x') !== 0 && \stripos($newContent, 'e') !== \false || \strpos($newContent, '.') !== \false) || \strpos($newContent, '0') === 0 && \stripos($newContent, '0x') !== 0 && \stripos($newContent, '0b') !== 0 && \octdec(\str_replace('_', '', $newContent)) > \PHP_INT_MAX || \strpos($newContent, '0') !== 0 && \str_replace('_', '', $newContent) > \PHP_INT_MAX)) {
                    $newType = \T_DNUMBER;
                }
                $newToken = [];
                $newToken['code'] = $newType;
                $newToken['type'] = \PHP_CodeSniffer\Util\Tokens::tokenName($newType);
            ...
            */
            if ($tokenIsArray === \false && $token[0] === '?') {
                ...
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                    if (\is_array($tokens[$i]) === \true) {
                        $tokenType = $tokens[$i][0];
                    } else {
                        $tokenType = $tokens[$i];
                    }
                    if (isset(\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokenType]) === \true) {
                        continue;
                    }
                ...
                for ($i = $stackPtr - 1; $i >= 0; $i--) {
                    ...
                    if ($prevNonEmpty === null && isset(\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokenType]) === \false) {
                    ...
                    if ($tokenType === \T_FUNCTION || $tokenType === \T_FN || isset(\PHP_CodeSniffer\Util\Tokens::$methodPrefixes[$tokenType]) === \true || $tokenType === \T_VAR) {
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            echo "\t\t* token {$stackPtr} changed from ? to T_NULLABLE" . \PHP_EOL;
                        }
                        $newToken['code'] = T_NULLABLE;
                        ...
                        if (\in_array($tokenType, [\T_DOUBLE_ARROW, \T_OPEN_TAG, \T_OPEN_TAG_WITH_ECHO, '=', '{', ';'], \true) === \true) {
                            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                                echo "\t\t* token {$stackPtr} changed from ? to T_INLINE_THEN" . \PHP_EOL;
                            }
                            $newToken['code'] = T_INLINE_THEN;
                    ...
                    if (isset(\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokenType]) === \false) {
                        $lastSeenNonEmpty = $tokenType;
                    }
                }
                //end for
            ...
            */
            if ($stackPtr > 1 && (\is_array($tokens[$stackPtr - 1]) === \true && $tokens[$stackPtr - 1][0] === \T_PAAMAYIM_NEKUDOTAYIM) && $tokenIsArray === \true && $token[0] !== \T_STRING && $token[0] !== \T_VARIABLE && $token[0] !== T_DOLLAR && isset(\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$token[0]]) === \false) {
                $newToken = [];
                $newToken['code'] = \T_STRING;
                $newToken['type'] = 'T_STRING';
                $newToken['content'] = $token[1];
            ...
            */
            if ($tokenIsArray === \true && $token[0] === \T_STRING && \strtolower($token[1]) === 'fn') {
            ...
            */
                ...
                */
                ...
                if ($parenthesisCloser !== \false) {
                    for ($x = $parenthesisCloser + 1; $x < $numTokens; $x++) {
                        if (\is_array($tokens[$x]) === \false || isset(\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$x][0]]) === \false) {
                            // Non-empty content.
                            if (\is_array($tokens[$x]) === \true && $tokens[$x][0] === \T_USE) {
                    ...
                    if (isset($tokens[$x]) === \true && \is_array($tokens[$x]) === \false && $tokens[$x] === ':') {
                        ...
                        for ($x += 1; $x < $numTokens; $x++) {
                            if (\is_array($tokens[$x]) === \true && isset(\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$x][0]]) === \true) {
            ...
            /*
                ...
                So look for and combine these tokens in earlier versions.
            */
            if ($tokenIsArray === \true && $token[0] === \T_IS_SMALLER_OR_EQUAL && isset($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === '>') {
                $newToken = [];
            ...
            */
            if ($tokenIsArray === \true && $token[0] === \T_STRING && isset($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1] === ':' && (\is_array($tokens[$stackPtr - 1]) === \false || $tokens[$stackPtr - 1][0] !== \T_PAAMAYIM_NEKUDOTAYIM)) {
                ...
                if ($finalTokens[$x]['code'] !== \T_CASE && $finalTokens[$x]['code'] !== T_INLINE_THEN) {
                    $finalTokens[$newStackPtr] = ['content' => $token[1] . ':', 'code' => T_GOTO_LABEL, 'type' => 'T_GOTO_LABEL'];
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
            ...
            */
            if ($tokenIsArray === \true && \strpos($token[1], $this->eolChar) !== \false) {
                $tokenLines = \explode($this->eolChar, $token[1]);
                $numLines = \count($tokenLines);
                $newToken = ['type' => \PHP_CodeSniffer\Util\Tokens::tokenName($token[0]), 'code' => $token[0], 'content' => ''];
                ...
                if ($tokenIsArray === \true && $token[0] === \T_STRING) {
                    ...
                    if (isset($context[$finalTokens[$lastNotEmptyToken]['code']]) === \true) {
                        ...
                        if ($finalTokens[$lastNotEmptyToken]['code'] === \T_NEW && \strtolower($token[1]) === 'self') {
                            $finalTokens[$newStackPtr] = ['content' => $token[1], 'code' => T_SELF, 'type' => 'T_SELF'];
                        } else {
                            $finalTokens[$newStackPtr] = ['content' => $token[1], 'code' => \T_STRING, 'type' => 'T_STRING'];
                        }
                        $newStackPtr++;
                        continue;
                    }
                ...
                $newToken = null;
                if ($tokenIsArray === \false) {
                    if (isset(self::$resolveTokenCache[$token[0]]) === \true) {
                    ...
                    if ($cacheKey !== null && isset(self::$resolveTokenCache[$cacheKey]) === \true) {
                ...
                if (($newToken['code'] === \T_CLASS || $newToken['code'] === \T_FUNCTION) && $finalTokens[$lastNotEmptyToken]['code'] === \T_DOUBLE_COLON) {
                    $newToken['code'] = \T_STRING;
                    $newToken['type'] = 'T_STRING';
                }
                ...
                // and T_CONST.
                if (($newToken['code'] === \T_FUNCTION || $newToken['code'] === \T_CONST) && ($finalTokens[$lastNotEmptyToken]['code'] === \T_USE || $insideUseGroup === \true)) {
                    $newToken['code'] = \T_STRING;
                    $newToken['type'] = 'T_STRING';
                }
                ...
                if ($insideUseGroup === \true && $newToken['code'] === T_CLOSE_CURLY_BRACKET) {
    ...
    protected function processAdditional()
    {
        ...
        for ($i = $numTokens - 1; $i >= 0; $i--) {
            ...
            if (isset($this->tokens[$i]['scope_opener']) === \true && isset($this->tokens[$i]['scope_condition']) === \false) {
            ...
            if ($this->tokens[$i]['code'] === \T_FUNCTION) {
                ...
                */
                if (isset($this->tokens[$i]['scope_opener']) === \true) {
                    for ($x = $i + 1; $x < $numTokens; $x++) {
                        if (isset(\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$x]['code']]) === \false && $this->tokens[$x]['code'] !== T_BITWISE_AND) {
                            break;
                        }
                    }
                ...
                if ($this->tokens[$i]['code'] === \T_CLASS && isset($this->tokens[$i]['scope_opener']) === \true) {
                    /*
                    ...
                    */
                    ...
                    if ($this->tokens[$x]['code'] === T_OPEN_PARENTHESIS || $this->tokens[$x]['code'] === T_OPEN_CURLY_BRACKET || $this->tokens[$x]['code'] === \T_EXTENDS || $this->tokens[$x]['code'] === \T_IMPLEMENTS) {
                        ...
                        if ($this->tokens[$x]['code'] === T_OPEN_PARENTHESIS && isset($this->tokens[$x]['parenthesis_closer']) === \true) {
                    ...
                    if ($this->tokens[$i]['code'] === \T_FN && isset($this->tokens[$i + 1]) === \true) {
                        ...
                        if (isset($this->tokens[$x]) === \true && $this->tokens[$x]['code'] === T_OPEN_PARENTHESIS) {
                            ...
                            if ($this->tokens[$arrow]['code'] === \T_DOUBLE_ARROW) {
                                $endTokens = [T_COLON => \true, T_COMMA => \true, T_SEMICOLON => \true, T_CLOSE_PARENTHESIS => \true, T_CLOSE_SQUARE_BRACKET => \true, T_CLOSE_CURLY_BRACKET => \true, T_CLOSE_SHORT_ARRAY => \true, \T_OPEN_TAG => \true, \T_CLOSE_TAG => \true];
                                $inTernary = \false;
                                $lastEndToken = null;
                                for ($scopeCloser = $arrow + 1; $scopeCloser < $numTokens; $scopeCloser++) {
                                    ...
                                    if (isset($this->tokens[$scopeCloser]['scope_closer']) === \true && $this->tokens[$scopeCloser]['code'] !== T_INLINE_ELSE && $this->tokens[$scopeCloser]['code'] !== \T_END_HEREDOC && $this->tokens[$scopeCloser]['code'] !== T_END_NOWDOC) {
                                        // We minus 1 here in case the closer can be shared with us.
                                        $scopeCloser = $this->tokens[$scopeCloser]['scope_closer'] - 1;
                                        continue;
                                    }
                                    if (isset($this->tokens[$scopeCloser]['parenthesis_closer']) === \true) {
                                        $scopeCloser = $this->tokens[$scopeCloser]['parenthesis_closer'];
                                        $lastEndToken = $scopeCloser;
                                        continue;
                                    }
                                    if (isset($this->tokens[$scopeCloser]['bracket_closer']) === \true) {
                        ...
                        if ($this->tokens[$i]['code'] === T_OPEN_SQUARE_BRACKET) {
                            ...
                            for ($x = $i - 1; $x >= 0; $x--) {
                                ...
                                if (isset($this->tokens[$x]['scope_opener']) === \true) {
                                    $isShortArray = \true;
                                    break;
                                }
                                if (isset(\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$x]['code']]) === \false) {
            ...
            if ($this->tokens[$x]['code'] !== T_OPEN_CURLY_BRACKET || isset($this->tokens[$x]['scope_condition']) === \true) {
            ...
            if ($this->tokens[$scopeCloser]['scope_condition'] === $i) {
                ...
                for ($y = $start; $y < $scopeCloser; $y++) {
                    if (isset($this->tokens[$y]['scope_closer']) === \true && $this->tokens[$y]['scope_closer'] === $scopeCloser) {
                    ...
                    if ($oldCond === $this->tokens[$i]['code']) {
                        ...
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            ...
                            foreach ($oldConditions as $condition) {
                                $oldConds .= \PHP_CodeSniffer\Util\Tokens::tokenName($condition) . ',';
                            }
                            $oldConds = \rtrim($oldConds, ',');
                            $newConds = '';
                            foreach ($this->tokens[$x]['conditions'] as $condition) {
                                $newConds .= \PHP_CodeSniffer\Util\Tokens::tokenName($condition) . ',';
                            }
    ...
    public static function standardiseToken($token)
    {
        ...
        if ($token[0] === \T_STRING) {
            ...
            if ($token[0] === \T_CURLY_OPEN) {
                $newToken = ['code' => T_OPEN_CURLY_BRACKET, 'type' => 'T_OPEN_CURLY_BRACKET'];
            } else {
                $newToken = ['code' => $token[0], 'type' => \PHP_CodeSniffer\Util\Tokens::tokenName($token[0])];
