...
class File
{
    ...
    public function __construct($path, \PHP_CodeSniffer\Ruleset $ruleset, \PHP_CodeSniffer\Config $config)
    {
        ...
        $this->config = $config;
        $this->fixer = new \PHP_CodeSniffer\Fixer();
        $parts = \explode('.', $path);
        $extension = \array_pop($parts);
        if (isset($config->extensions[$extension]) === \true) {
    ...
    public function process()
    {
        ...
        foreach ($this->tokens as $stackPtr => $token) {
                ...
                if (\strpos($commentText, '@codingStandards') !== \false) {
                        ...
                        if (\strpos($commentText, '@codingStandardsChangeSetting') !== \false) {
                            ...
                            if (\count($parts) >= 2) {
                                ...
                                if (\count($sniffParts) >= 3) {
                                    ...
                                    if (isset($this->ruleset->sniffCodes[$parts[0]]) === \true) {
                    ...
                    if (\substr($commentTextLower, 0, 16) === 'phpcs:ignorefile' || \substr($commentTextLower, 0, 17) === '@phpcs:ignorefile') {
                        ...
                        if (\substr($commentTextLower, 0, 9) === 'phpcs:set' || \substr($commentTextLower, 0, 10) === '@phpcs:set') {
                            if (isset($token['sniffCode']) === \true) {
                                $listenerCode = $token['sniffCode'];
                                if (isset($this->ruleset->sniffCodes[$listenerCode]) === \true) {
            ...
            if (PHP_CODESNIFFER_VERBOSITY > 2) {
                $type = $token['type'];
                $content = \PHP_CodeSniffer\Util\Common::prepareForOutput($token['content']);
            ...
            foreach ($this->ruleset->tokenListeners[$token['code']] as $listenerData) {
                if (isset($this->ignoredListeners[$listenerData['class']]) === \true || isset($listenerIgnoreTo[$listenerData['class']]) === \true && $listenerIgnoreTo[$listenerData['class']] > $stackPtr) {
                ...
                if (\trim($this->path, '\'"') !== 'STDIN') {
                    ...
                    foreach ($listenerData['ignore'] as $pattern) {
                        ...
                        if (\preg_match($pattern, $this->path) === 1) {
                            $this->ignoredListeners[$class] = \true;
                            continue 2;
                        }
                    }
        ...
        if (PHP_CODESNIFFER_VERBOSITY > 2) {
            ...
            $this->listenerTimes = \array_reverse($this->listenerTimes, \true);
    ...
    public function addError($error, $stackPtr, $code, $data = [], $severity = 0, $fixable = \false)
    {
        ...
        return $this->addMessage(\true, $error, $line, $column, $code, $data, $severity, $fixable);
    }
    //end addError()
    /**
    ...
    protected function addMessage($error, $message, $line, $column, $code, $data, $severity, $fixable)
    {
        ...
        if ($parts[0] === 'Internal') {
            // An internal message.
            $listenerCode = \PHP_CodeSniffer\Util\Common::getSniffCode($this->activeListener);
            $sniffCode = $code;
            ...
            $checkCodes = [$sniffCode, $parts[0] . '.' . $parts[1] . '.' . $parts[2], $parts[0] . '.' . $parts[1], $parts[0]];
        }
        //end if
        if (isset($this->tokenizer->ignoredLines[$line]) === \true) {
            // Check if this line is ignoring this specific message.
            $ignored = \false;
            foreach ($checkCodes as $checkCode) {
                if (isset($this->tokenizer->ignoredLines[$line][$checkCode]) === \true) {
                    $ignored = \true;
                    break;
                }
            }
            // If it is ignored, make sure it's not whitelisted.
            if ($ignored === \true && isset($this->tokenizer->ignoredLines[$line]['.except']) === \true) {
                foreach ($checkCodes as $checkCode) {
                    if (isset($this->tokenizer->ignoredLines[$line]['.except'][$checkCode]) === \true) {
                        $ignored = \false;
                        break;
                    }
                }
        ...
        if ($includeAll === \false && (empty($this->configCache['sniffs']) === \false && \in_array(\strtolower($listenerCode), $this->configCache['sniffs'], \true) === \false || empty($this->configCache['exclude']) === \false && \in_array(\strtolower($listenerCode), $this->configCache['exclude'], \true) === \true)) {
            return \false;
        }
        // If we know this sniff code is being ignored for this file, return early.
        foreach ($checkCodes as $checkCode) {
            if (isset($this->ignoredCodes[$checkCode]) === \true) {
                return \false;
            }
        }
        ...
        foreach ($checkCodes as $checkCode) {
            ...
            if (isset($this->ruleset->ruleset[$checkCode]['type']) === \true && $this->ruleset->ruleset[$checkCode]['type'] === $oppositeType) {
                $error = !$error;
                break;
            }
        }
        ...
        foreach ($checkCodes as $checkCode) {
            ...
            if (isset($this->ruleset->ruleset[$checkCode]['severity']) === \true) {
        ...
        if (\trim($this->path, '\'"') === 'STDIN') {
            ...
            foreach ($checkCodes as $checkCode) {
                $patterns = null;
                if (isset($this->configCache['includePatterns'][$checkCode]) === \true) {
                    $patterns = $this->configCache['includePatterns'][$checkCode];
                    $excluding = \false;
                } else {
                    if (isset($this->configCache['ignorePatterns'][$checkCode]) === \true) {
                ...
                foreach ($patterns as $pattern => $type) {
                    ...
                    if ($excluding === \true) {
                        ...
                        $this->ignoredCodes[$checkCode] = \true;
                        return \false;
                    }
        ...
        // product replaced.
        if ($this->replayingErrors === \false && isset($this->ruleset->ruleset[$sniffCode]['message']) === \true) {
    ...
    public function recordMetric($stackPtr, $metric, $value)
    {
        if (isset($this->metrics[$metric]) === \false) {
            ...
            if (isset($this->metricTokens[$metric][$stackPtr]) === \false) {
                $this->metricTokens[$metric][$stackPtr] = \true;
    ...
    public function getDeclarationName($stackPtr)
    {
        ...
        if ($tokenCode !== \T_FUNCTION && $tokenCode !== \T_CLASS && $tokenCode !== \T_INTERFACE && $tokenCode !== \T_TRAIT) {
            throw new \PHP_CodeSniffer\Exceptions\RuntimeException('Token type "' . $this->tokens[$stackPtr]['type'] . '" is not T_FUNCTION, T_CLASS, T_INTERFACE or T_TRAIT');
        }
    ...
    public function getMethodParameters($stackPtr)
    {
        ...
        if ($this->tokens[$stackPtr]['code'] === \T_USE) {
            ...
            if ($opener === \false || isset($this->tokens[$opener]['parenthesis_owner']) === \true) {
                throw new \PHP_CodeSniffer\Exceptions\RuntimeException('$stackPtr was not a valid T_USE');
            }
        } else {
        ...
        for ($i = $paramStart; $i <= $closer; $i++) {
            ...
            switch ($this->tokens[$i]['code']) {
                ...
                case T_COMMA:
                    ...
                    $vars[$paramCount]['content'] = \trim($this->getTokensAsString($paramStart, $i - $paramStart));
                    if ($defaultStart !== null) {
                        $vars[$paramCount]['default'] = \trim($this->getTokensAsString($defaultStart, $i - $defaultStart));
                    ...
                    break;
                case T_EQUAL:
                    $defaultStart = $this->findNext(\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true);
                    $equalToken = $i;
                    break;
            }
            //end switch
    ...
    public function getMethodProperties($stackPtr)
    {
        ...
        $hasBody = \true;
        if (isset($this->tokens[$stackPtr]['parenthesis_closer']) === \true) {
            $scopeOpener = null;
            if (isset($this->tokens[$stackPtr]['scope_opener']) === \true) {
                $scopeOpener = $this->tokens[$stackPtr]['scope_opener'];
            }
            $valid = [\T_STRING => \T_STRING, \T_CALLABLE => \T_CALLABLE, T_SELF => T_SELF, T_PARENT => T_PARENT, \T_STATIC => \T_STATIC, T_FALSE => T_FALSE, T_NULL => T_NULL, \T_NAMESPACE => \T_NAMESPACE, \T_NS_SEPARATOR => \T_NS_SEPARATOR, T_TYPE_UNION => T_TYPE_UNION];
            for ($i = $this->tokens[$stackPtr]['parenthesis_closer']; $i < $this->numTokens; $i++) {
                ...
                if (isset($valid[$this->tokens[$i]['code']]) === \true) {
        ...
        //end if
        if ($returnType !== '' && $nullableReturnType === \true) {
            $returnType = '?' . $returnType;
        }
        return ['scope' => $scope, 'scope_specified' => $scopeSpecified, 'return_type' => $returnType, 'return_type_token' => $returnTypeToken, 'return_type_end_token' => $returnTypeEndToken, 'nullable_return_type' => $nullableReturnType, 'is_abstract' => $isAbstract, 'is_final' => $isFinal, 'is_static' => $isStatic, 'has_body' => $hasBody];
    }
    ...
    public function getMemberProperties($stackPtr)
    {
        ...
        if (isset($this->tokens[$ptr]) === \false || $this->tokens[$ptr]['code'] !== \T_CLASS && $this->tokens[$ptr]['code'] !== T_ANON_CLASS && $this->tokens[$ptr]['code'] !== \T_TRAIT) {
            if (isset($this->tokens[$ptr]) === \true && $this->tokens[$ptr]['code'] === \T_INTERFACE) {
                // T_VARIABLEs in interfaces can actually be method arguments
                // but they wont be seen as being inside the method because there
                // are no scope openers and closers for abstract methods. If it is in
                // parentheses, we can be pretty sure it is a method argument.
                if (isset($this->tokens[$stackPtr]['nested_parenthesis']) === \false || empty($this->tokens[$stackPtr]['nested_parenthesis']) === \true) {
                    $error = 'Possible parse error: interfaces may not include member vars';
                    $this->addWarning($error, $stackPtr, 'Internal.ParseError.InterfaceHasMemberVar');
                    return [];
                }
            } else {
                throw new \PHP_CodeSniffer\Exceptions\RuntimeException('$stackPtr is not a class member var');
            }
        }
        ...
        $valid = [\T_PUBLIC => \T_PUBLIC, \T_PRIVATE => \T_PRIVATE, \T_PROTECTED => \T_PROTECTED, \T_STATIC => \T_STATIC, \T_VAR => \T_VAR];
        $valid += \PHP_CodeSniffer\Util\Tokens::$emptyTokens;
        $scope = 'public';
        $scopeSpecified = \false;
        $isStatic = \false;
        $startOfStatement = $this->findPrevious([T_SEMICOLON, T_OPEN_CURLY_BRACKET, T_CLOSE_CURLY_BRACKET, T_ATTRIBUTE_END], $stackPtr - 1);
        ...
        if ($i < $stackPtr) {
            // We've found a type.
            $valid = [\T_STRING => \T_STRING, \T_CALLABLE => \T_CALLABLE, T_SELF => T_SELF, T_PARENT => T_PARENT, T_FALSE => T_FALSE, T_NULL => T_NULL, \T_NAMESPACE => \T_NAMESPACE, \T_NS_SEPARATOR => \T_NS_SEPARATOR, T_TYPE_UNION => T_TYPE_UNION];
            for ($i; $i < $stackPtr; $i++) {
            ...
            if ($type !== '' && $nullableType === \true) {
                $type = '?' . $type;
            }
        }
        //end if
        return ['scope' => $scope, 'scope_specified' => $scopeSpecified, 'is_static' => $isStatic, 'type' => $type, 'type_token' => $typeToken, 'type_end_token' => $typeEndToken, 'nullable_type' => $nullableType];
    }
    ...
    public function getClassProperties($stackPtr)
    {
        ...
        $valid = [\T_FINAL => \T_FINAL, \T_ABSTRACT => \T_ABSTRACT, \T_WHITESPACE => \T_WHITESPACE, \T_COMMENT => \T_COMMENT, \T_DOC_COMMENT => \T_DOC_COMMENT];
        $isAbstract = \false;
        $isFinal = \false;
        for ($i = $stackPtr - 1; $i > 0; $i--) {
            if (isset($valid[$this->tokens[$i]['code']]) === \false) {
        ...
        //end for
        return ['is_abstract' => $isAbstract, 'is_final' => $isFinal];
    }
    ...
    public function isReference($stackPtr)
    {
        ...
        $tokenBefore = $this->findPrevious(\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        ...
        if (isset(\PHP_CodeSniffer\Util\Tokens::$assignmentTokens[$this->tokens[$tokenBefore]['code']]) === \true) {
            // This is directly after an assignment. It's a reference. Even if
            // it is part of an operation, the other tests will handle it.
            return \true;
        }
        $tokenAfter = $this->findNext(\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if ($this->tokens[$tokenAfter]['code'] === \T_NEW) {
            return \true;
        }
        if (isset($this->tokens[$stackPtr]['nested_parenthesis']) === \true) {
        ...
        if ($this->tokens[$tokenBefore]['code'] === T_OPEN_PARENTHESIS || $this->tokens[$tokenBefore]['code'] === T_COMMA || $this->tokens[$tokenBefore]['code'] === T_OPEN_SHORT_ARRAY) {
            if ($this->tokens[$tokenAfter]['code'] === \T_VARIABLE) {
                ...
                $skip = \PHP_CodeSniffer\Util\Tokens::$emptyTokens;
                $skip[] = \T_NS_SEPARATOR;
                $skip[] = T_SELF;
                $skip[] = T_PARENT;
                $skip[] = \T_STATIC;
                ...
                $skip[] = \T_NAMESPACE;
                $skip[] = \T_DOUBLE_COLON;
                $nextSignificantAfter = $this->findNext($skip, $stackPtr + 1, null, \true);
    ...
    public function getTokensAsString($start, $length, $origContent = \false)
    {
        ...
        for ($i = $start; $i < $end; $i++) {
            ...
            if ($origContent === \true && isset($this->tokens[$i]['orig_content']) === \true) {
    ...
    public function findPrevious($types, $start, $end = null, $exclude = \false, $value = null, $local = \false)
    {
        ...
        for ($i = $start; $i >= $end; $i--) {
            ...
            if ($local === \true) {
                if (isset($this->tokens[$i]['scope_opener']) === \true && $i === $this->tokens[$i]['scope_closer']) {
                    $i = $this->tokens[$i]['scope_opener'];
                } else {
                    if (isset($this->tokens[$i]['bracket_opener']) === \true && $i === $this->tokens[$i]['bracket_closer']) {
                        $i = $this->tokens[$i]['bracket_opener'];
                    } else {
                        if (isset($this->tokens[$i]['parenthesis_opener']) === \true && $i === $this->tokens[$i]['parenthesis_closer']) {
    ...
    public function findNext($types, $start, $end = null, $exclude = \false, $value = null, $local = \false)
    {
        ...
        for ($i = $start; $i < $end; $i++) {
            ...
            if ($local === \true && $this->tokens[$i]['code'] === T_SEMICOLON) {
                break;
            }
        }
        //end for
    ...
    public function findStartOfStatement($start, $ignore = null)
    {
        ...
        if ($ignore !== null) {
            ...
            foreach ($ignore as $code) {
                if (isset($startTokens[$code]) === \true) {
                    unset($startTokens[$code]);
                }
                if (isset($endTokens[$code]) === \true) {
        ...
        if ($matchExpression !== \false) {
            ...
            if ($prevMatch <= $this->tokens[$matchExpression]['scope_opener']) {
                ...
                $next = $this->findNext(\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $this->tokens[$matchExpression]['scope_opener'] + 1, null, \true);
                if ($next === \false) {
                    return $start;
                }
                return $next;
            ...
            if ($this->tokens[$prevMatch]['code'] === T_COMMA) {
                ...
                $next = $this->findNext(\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $end + 1, null, \true);
                return $next;
            }
        }
        //end if
        ...
        while (isset($endTokens[$this->tokens[$start]['code']]) === \true || isset($this->tokens[$start]['scope_condition']) === \true && $start === $this->tokens[$start]['scope_closer']) {
            if (isset($this->tokens[$start]['scope_condition']) === \true) {
        ...
        for ($i = $start; $i >= 0; $i--) {
            if (isset($startTokens[$this->tokens[$i]['code']]) === \true || isset($endTokens[$this->tokens[$i]['code']]) === \true) {
            ...
            //end if
            if (isset(\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$i]['code']]) === \false) {
                $lastNotEmpty = $i;
            }
        }
        //end for
    ...
    public function findEndOfStatement($start, $ignore = null)
    {
        ...
        for ($i = $start; $i < $this->numTokens; $i++) {
            if ($i !== $start && isset($endTokens[$this->tokens[$i]['code']]) === \true) {
            ...
            if (isset($this->tokens[$i]['scope_closer']) === \true && ($i === $this->tokens[$i]['scope_opener'] || $i === $this->tokens[$i]['scope_condition'])) {
                ...
                if ($i === $start && isset(\PHP_CodeSniffer\Util\Tokens::$scopeOpeners[$this->tokens[$i]['code']]) === \true) {
                    return $this->tokens[$i]['scope_closer'];
                }
                $i = $this->tokens[$i]['scope_closer'];
            } else {
                if (isset($this->tokens[$i]['bracket_closer']) === \true && $i === $this->tokens[$i]['bracket_opener']) {
                    $i = $this->tokens[$i]['bracket_closer'];
                } else {
                    if (isset($this->tokens[$i]['parenthesis_closer']) === \true && $i === $this->tokens[$i]['parenthesis_opener']) {
    ...
    public function hasCondition($stackPtr, $types)
    {
        ...
        foreach ($types as $type) {
            if (\in_array($type, $conditions, \true) === \true) {
    ...
    public function findExtendedClassName($stackPtr)
    {
        ...
        $end = $this->findNext($find, $extendsIndex + 1, $classOpenerIndex + 1, \true);
    ...
    public function findImplementedInterfaceNames($stackPtr)
    {
        ...
        if ($this->tokens[$stackPtr]['code'] !== \T_CLASS && $this->tokens[$stackPtr]['code'] !== T_ANON_CLASS) {
            return \false;
        }
        ...
        $end = $this->findNext($find, $implementsIndex + 1, $classOpenerIndex + 1, \true);
