...
class Runner
{
    ...
    public function runPHPCS()
    {
        try {
            ...
            if ($this->config->interactive === \true) {
    ...
    private function run()
    {
        ...
        if ($this->config->stdin === \true) {
            ...
            if ($fileContents === null) {
                ...
                \stream_set_blocking($handle, \true);
            ...
            for ($batch = 0; $batch < $this->config->parallel; $batch++) {
                ...
                if ($pid === -1) {
                    ...
                    if ($pid !== 0) {
                        ...
                        $childOutput = ['totalFiles' => $this->reporter->totalFiles, 'totalErrors' => $this->reporter->totalErrors, 'totalWarnings' => $this->reporter->totalWarnings, 'totalFixable' => $this->reporter->totalFixable, 'totalFixed' => $this->reporter->totalFixed];
                        $output = '<' . '?php' . "\n" . ' $childOutput = ';
                        $output .= \var_export($childOutput, \true);
                        $output .= ";\n\$debugOutput = ";
                        $output .= \var_export($debugOutput, \true);
                        if ($this->config->cache === \true) {
                            $childCache = [];
